...
public class UTF8JsonGenerator extends JsonGeneratorImpl {
        ...
        while (offset < inputEnd) {
            do {
                ...
                if (ch >= 128) {
                    ...
                    char chx = cbuf[offset++];
                    if (chx < 2048) {
                        bbuf[this._outputTail++] = (byte)(192 | chx >> 6);
                        bbuf[this._outputTail++] = (byte)(128 | chx & 63);
                    } else {
                        offset = this._outputRawMultiByteChar(chx, cbuf, offset, inputEnd);
                    }
                    continue label33;
                }
        ...
        while (offset < end) {
            do {
                ...
                if (ch > 127) {
                    char chx = cbuf[offset++];
                    if (chx < 2048) {
                        this._outputBuffer[this._outputTail++] = (byte)(192 | chx >> 6);
                        this._outputBuffer[this._outputTail++] = (byte)(128 | chx & 63);
                        break;
                    }

                    offset = this._outputRawMultiByteChar(chx, cbuf, offset, end);
                    break;
                }
    ...
    public void writeNumber(double d) throws IOException {
        if (!this._cfgNumbersAsStrings
            && (!Double.isNaN(d) && !Double.isInfinite(d) || !JsonGenerator.Feature.QUOTE_NON_NUMERIC_NUMBERS.enabledIn(this._features))) {
            this._verifyValueWrite("write a number");
    ...
    public void writeNumber(float f) throws IOException {
        if (!this._cfgNumbersAsStrings
            && (!Float.isNaN(f) && !Float.isInfinite(f) || !JsonGenerator.Feature.QUOTE_NON_NUMERIC_NUMBERS.enabledIn(this._features))) {
            this._verifyValueWrite("write a number");
