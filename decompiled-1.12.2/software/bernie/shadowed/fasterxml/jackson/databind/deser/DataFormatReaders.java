...
public class DataFormatReaders {
    ...
    private DataFormatReaders.Match _findFormat(DataFormatReaders.AccessorForReader acc) throws IOException {
        ...
        for (ObjectReader f : this._readers) {
            ...
            if (strength != null
                && strength.ordinal() >= this._minimalMatch.ordinal()
                && (bestMatch == null || bestMatchStrength.ordinal() < strength.ordinal())) {
                bestMatch = f;
    ...
    public static class Match {
        ...
        public JsonParser createParserWithMatch() throws IOException {
            ...
            return this._originalStream == null
                ? jf.createParser(this._bufferedData, this._bufferedStart, this._bufferedLength)
                : jf.createParser(this.getDataStream());
        }

        public InputStream getDataStream() {
            return this._originalStream == null
                ? new ByteArrayInputStream(this._bufferedData, this._bufferedStart, this._bufferedLength)
                : new MergedStream(null, this._originalStream, this._bufferedData, this._bufferedStart, this._bufferedLength);
        }
    }
}
