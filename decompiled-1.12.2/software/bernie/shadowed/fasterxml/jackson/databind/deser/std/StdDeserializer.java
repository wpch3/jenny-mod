...
public abstract class StdDeserializer<T> extends JsonDeserializer<T> implements Serializable {
    ...
    protected final boolean _parseBooleanPrimitive(JsonParser p, DeserializationContext ctxt) throws IOException {
        ...
        if (t == JsonToken.VALUE_STRING) {
            ...
            if ("true".equals(text) || "True".equals(text)) {
                ...
                return false;
            } else if (this._isEmptyOrTextualNull(text)) {
            ...
            if (text.length() > 9) {
                ...
                if (this._intOverflow(l)) {
                    Number v = (Number)ctxt.handleWeirdStringValue(
                        this._valueClass, text, "Overflow: numeric value (%s) out of range of int (%d -%d)", text, Integer.MIN_VALUE, Integer.MAX_VALUE
                    );
        ...
        switch (p.getCurrentTokenId()) {
            ...
            case 7:
                ...
                try {
                    ...
                    Number v = (Number)ctxt.handleWeirdNumberValue(
                        this._valueClass, p.getNumberValue(), "not a valid 64-bit long for creating `java.util.Date`"
                    );
                    ts = v.longValue();
                }
    ...
    protected Date _parseDateFromArray(JsonParser p, DeserializationContext ctxt) throws IOException {
        ...
        return (Date)ctxt.handleUnexpectedToken(this._valueClass, t, p, null);
    }
    ...
    protected T _deserializeFromArray(JsonParser p, DeserializationContext ctxt) throws IOException {
        ...
        if (ctxt.hasSomeOfFeatures(F_MASK_ACCEPT_ARRAYS)) {
            ...
            if (ctxt.isEnabled(DeserializationFeature.UNWRAP_SINGLE_VALUE_ARRAYS)) {
                T parsed = this.deserialize(p, ctxt);
        ...
        return (T)ctxt.handleUnexpectedToken(this._valueClass, t, p, null);
    }
    ...
    protected T _deserializeWrappedValue(JsonParser p, DeserializationContext ctxt) throws IOException {
        if (p.hasToken(JsonToken.START_ARRAY)) {
            ...
            return (T)ctxt.handleUnexpectedToken(this._valueClass, p.getCurrentToken(), p, msg);
        } else {
    ...
    protected final void _verifyNullForPrimitive(DeserializationContext ctxt) throws JsonMappingException {
        if (ctxt.isEnabled(DeserializationFeature.FAIL_ON_NULL_FOR_PRIMITIVES)) {
            ctxt.reportInputMismatch(
                this, "Cannot coerce `null` %s (disable `DeserializationFeature.FAIL_ON_NULL_FOR_PRIMITIVES` to allow)", this._coercedTypeDesc()
            );
        }
    }
    ...
    protected void _verifyStringForScalarCoercion(DeserializationContext ctxt, String str) throws JsonMappingException {
        ...
        if (!ctxt.isEnabled(feat)) {
            ctxt.reportInputMismatch(
                this, "Cannot coerce String \"%s\" %s (enable `%s.%s` to allow)", str, this._coercedTypeDesc(), feat.getClass().getSimpleName(), feat.name()
            );
        }
    }
    ...
    protected NullValueProvider findContentNullProvider(DeserializationContext ctxt, BeanProperty prop, JsonDeserializer<?> valueDeser) throws JsonMappingException {
        ...
        return prov != null ? prov : valueDeser;
    }
    ...
    protected final NullValueProvider _findNullProvider(DeserializationContext ctxt, BeanProperty prop, Nulls nulls, JsonDeserializer<?> valueDeser) throws JsonMappingException {
        if (nulls == Nulls.FAIL) {
            return prop == null
                ? NullsFailProvider.constructForRootValue(ctxt.constructType(valueDeser.handledType()))
                : NullsFailProvider.constructForProperty(prop);
        }
        ...
        if (nulls == Nulls.AS_EMPTY) {
            ...
            if (access == AccessPattern.ALWAYS_NULL) {
                ...
                return access == AccessPattern.CONSTANT ? NullsConstantProvider.forValue(valueDeser.getEmptyValue(ctxt)) : new NullsAsEmptyProvider(valueDeser);
            }
        } else {
