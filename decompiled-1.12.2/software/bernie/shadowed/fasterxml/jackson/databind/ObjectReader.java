...
public class ObjectReader extends ObjectCodec implements Versioned, Serializable {
    ...
    public ObjectReader with(InjectableValues injectableValues) {
        return this._injectableValues == injectableValues
            ? this
            : this._new(
                this, this._config, this._valueType, this._rootDeserializer, this._valueToUpdate, this._schema, injectableValues, this._dataFormatReaders
            );
    }
    ...
    public <T> T readValue(InputStream src) throws IOException {
        return (T)(this._dataFormatReaders != null
            ...
            : this._bindAndClose(this._considerFilter(this._parserFactory.createParser(src), false)));
    }
    ...
    public <T> T readValue(byte[] src) throws IOException {
        return (T)(this._dataFormatReaders != null
            ? this._detectBindAndClose(src, 0, src.length)
            : this._bindAndClose(this._considerFilter(this._parserFactory.createParser(src), false)));
    }

    public <T> T readValue(byte[] src, int offset, int length) throws IOException {
        return (T)(this._dataFormatReaders != null
            ? this._detectBindAndClose(src, offset, length)
            : this._bindAndClose(this._considerFilter(this._parserFactory.createParser(src, offset, length), false)));
    }
    ...
    public JsonNode readTree(InputStream in) throws IOException {
        return this._dataFormatReaders != null
            ? this._detectBindAndCloseAsTree(in)
            : this._bindAndCloseAsTree(this._considerFilter(this._parserFactory.createParser(in), false));
    }
    ...
    public <T> MappingIterator<T> readValues(InputStream src) throws IOException {
        return this._dataFormatReaders != null
            ...
            : this._bindAndReadValues(this._considerFilter(this._parserFactory.createParser(src), true));
    }
    ...
    public <T> MappingIterator<T> readValues(byte[] src, int offset, int length) throws IOException {
        return this._dataFormatReaders != null
            ...
            : this._bindAndReadValues(this._considerFilter(this._parserFactory.createParser(src, offset, length), true));
    }
    ...
    protected Object _unwrapAndDeserialize(JsonParser p, DeserializationContext ctxt, JavaType rootType, JsonDeserializer<Object> deser) throws IOException {
        ...
        if (p.nextToken() != JsonToken.FIELD_NAME) {
            ctxt.reportWrongTokenException(
                rootType, JsonToken.FIELD_NAME, "Current token not FIELD_NAME (to contain expected root name '%s'), but %s", expSimpleName, p.getCurrentToken()
            );
        }
    ...
    protected JsonParser _considerFilter(JsonParser p, boolean multiValue) {
        return this._filter != null && !FilteringParserDelegate.class.isInstance(p) ? new FilteringParserDelegate(p, this._filter, false, multiValue) : p;
    }
    ...
    protected void _reportUndetectableSource(Object src) throws JsonProcessingException {
        throw new JsonParseException(
            null, "Cannot use source of type " + src.getClass().getName() + " with format auto-detection: must be byte- not char-based"
        );
    }
    ...
    protected JsonDeserializer<Object> _prefetchRootDeserializer(JavaType valueType) {
        if (valueType != null && this._config.isEnabled(DeserializationFeature.EAGER_DESERIALIZER_FETCH)) {
            ...
            if (deser == null) {
                try {
                    DeserializationContext ctxt = this.createDeserializationContext(null);
