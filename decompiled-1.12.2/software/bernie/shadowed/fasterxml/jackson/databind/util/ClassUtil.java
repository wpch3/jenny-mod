...
public final class ClassUtil {
    ...
    public static boolean isCollectionMapOrArray(Class<?> type) {
        if (type.isArray()) {
            ...
            return Collection.class.isAssignableFrom(type) ? true : Map.class.isAssignableFrom(type);
        }
    }
    ...
    public static void verifyMustOverride(Class<?> expType, Object instance, String method) {
        if (instance.getClass() != expType) {
            throw new IllegalStateException(
                String.format("Sub-class %s (of class %s) must override method '%s'", instance.getClass().getName(), expType.getName(), method)
            );
        }
    }
    ...
    public static boolean hasGetterSignature(Method m) {
        ...
        return pts != null && pts.length != 0 ? false : void.class != m.getReturnType();
    }
    ...
    public static Class<?> primitiveType(Class<?> type) {
        if (type.isPrimitive()) {
            ...
            return type == Character.class ? char.class : null;
        }
    }
    ...
    public static void checkAndFixAccess(Member member, boolean force) {
        ...
        try {
            ...
            if (!ao.isAccessible()) {
                ...
                throw new IllegalArgumentException(
                    "Cannot access " + member + " (from class " + declClass.getName() + "; failed to set access: " + se.getMessage()
                );
            }
        }
    }
