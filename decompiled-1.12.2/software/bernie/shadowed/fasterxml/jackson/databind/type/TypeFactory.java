...
public final class TypeFactory implements Serializable {
    ...
    protected Class<?> _findPrimitive(String className) {
        if ("int".equals(className)) {
            ...
            return "void".equals(className) ? void.class : null;
        }
    }
            ...
            if (!realET.equals(elementType)) {
                throw new IllegalArgumentException(
                    String.format(
                        ...
                        ClassUtil.nameOf(collectionClass),
                        elementType,
                        realET
                    )
                );
    ...
    public MapType constructMapType(Class<? extends Map> mapClass, JavaType keyType, JavaType valueType) {
        ...
        if (bindings.isEmpty()) {
            ...
            if (!realKT.equals(keyType)) {
                throw new IllegalArgumentException(
                    String.format("Non-generic Map class %s did not resolve to something with key type %s but %s ", ClassUtil.nameOf(mapClass), keyType, realKT)
                );
            }
            ...
            if (!realVT.equals(valueType)) {
                throw new IllegalArgumentException(
                    String.format(
                        "Non-generic Map class %s did not resolve to something with value type %s but %s ", ClassUtil.nameOf(mapClass), valueType, realVT
                    )
                );
            }
        }
            ...
            for (TypeModifier mod : this._modifiers) {
                ...
                if (t == null) {
                    throw new IllegalStateException(
                        String.format("TypeModifier %s (of type %s) return null for type %s", mod, mod.getClass().getName(), resultType)
                    );
                }
