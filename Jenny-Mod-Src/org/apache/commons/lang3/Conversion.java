...
public class Conversion {
   ...
   public static char binaryBeMsb0ToHexDigit(boolean[] src, int srcPos) {
      ...
      int var6 = 0;
      if (src[var6]) {
         if (src.length > var6 + 1 && src[var6 + 1]) {
            if (src.length > var6 + 2 && src[var6 + 2]) {
               return (char)(src.length > var6 + 3 && src[var6 + 3] ? 'f' : 'e');
            } else {
               return (char)(src.length > var6 + 3 && src[var6 + 3] ? 'd' : 'c');
            }
         } else if (src.length > var6 + 2 && src[var6 + 2]) {
            return (char)(src.length > var6 + 3 && src[var6 + 3] ? 'b' : 'a');
         } else {
            return (char)(src.length > var6 + 3 && src[var6 + 3] ? '9' : '8');
         }
      } else if (src.length > var6 + 1 && src[var6 + 1]) {
         if (src.length > var6 + 2 && src[var6 + 2]) {
            return (char)(src.length > var6 + 3 && src[var6 + 3] ? '7' : '6');
         } else {
            return (char)(src.length > var6 + 3 && src[var6 + 3] ? '5' : '4');
         }
      } else if (src.length > var6 + 2 && src[var6 + 2]) {
         return (char)(src.length > var6 + 3 && src[var6 + 3] ? '3' : '2');
      } else {
         return (char)(src.length > var6 + 3 && src[var6 + 3] ? '1' : '0');
      }
   }
   ...
   public static long shortArrayToLong(short[] src, int srcPos, long dstInit, int dstPos, int nShorts) {
      if ((src.length != 0 || srcPos != 0) && 0 != nShorts) {
         ...
         for (int i = 0; i < nShorts; i++) {
            ...
            long bits = (65535L & src[i + srcPos]) << shift;
   ...
   public static long byteArrayToLong(byte[] src, int srcPos, long dstInit, int dstPos, int nBytes) {
      if ((src.length != 0 || srcPos != 0) && 0 != nBytes) {
         ...
         for (int i = 0; i < nBytes; i++) {
            ...
            long bits = (255L & src[i + srcPos]) << shift;
   ...
   public static long hexToLong(String src, int srcPos, long dstInit, int dstPos, int nHex) {
      ...
      for (int i = 0; i < nHex; i++) {
         ...
         long bits = (15L & hexDigitToInt(src.charAt(i + srcPos))) << shift;
         long mask = 15L << shift;
         out = out & ~mask | bits;
      }
