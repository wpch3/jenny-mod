...
final class CharRange implements Iterable<Character>, Serializable {
   ...
   public boolean contains(CharRange range) {
      ...
      if (this.negated) {
         return range.negated ? this.start >= range.start && this.end <= range.end : range.end < this.start || range.start > this.end;
      } else {
         return range.negated ? this.start == 0 && this.end == '\uffff' : this.start <= range.start && this.end >= range.end;
      }
   }
