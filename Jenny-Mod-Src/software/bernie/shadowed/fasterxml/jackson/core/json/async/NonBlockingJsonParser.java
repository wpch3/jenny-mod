...
public class NonBlockingJsonParser extends NonBlockingJsonParserBase implements ByteArrayFeeder {
   ...
   public void feedInput(byte[] buf, int start, int end) throws IOException {
      ...
      this._currInputProcessed = this._currInputProcessed + this._origBufferLen;
   ...
   protected JsonToken _startFloat(char[] outBuf, int outPtr, int ch) throws IOException {
      ...
      if (ch == 46) {
         ...
         while (true) {
            ...
            int var6 = this._inputBuffer[this._inputPtr++];
            if (var6 < 48 || var6 > 57) {
               ch = var6 & 255;
               if (fractLen == 0) {
      ...
      if (ch == 101 || ch == 69) {
         ...
         int var7 = this._inputBuffer[this._inputPtr++];
         if (var7 == 45 || var7 == 43) {
            if (outPtr >= outBuf.length) {
               outBuf = this._textBuffer.expandCurrentSegment();
            }

            outBuf[outPtr++] = (char)var7;
            if (this._inputPtr >= this._inputEnd) {
               this._textBuffer.setCurrentLength(outPtr);
               this._minorState = 32;
               this._expLength = 0;
            ...
            var7 = this._inputBuffer[this._inputPtr++];
         }
         ...
         var7 &= 255;
         if (expLen == 0) {
            this.reportUnexpectedNumberChar(var7, "Exponent indicator not followed by a digit");
         }
      }
   ...
   protected final JsonToken _finishFieldWithEscape() throws IOException {
      ...
      return this._minorStateAfterSplit == 9
         ? this._finishAposName(this._quadLength, currQuad, currQuadBytes)
         : this._parseEscapedName(this._quadLength, currQuad, currQuadBytes);
   }
