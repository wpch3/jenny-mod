...
public abstract class JsonGenerator implements Closeable, Flushable, Versioned {
   ...
   public void setSchema(FormatSchema schema) {
      throw new UnsupportedOperationException(
         "Generator of type " + this.getClass().getName() + " does not support schema of type '" + schema.getSchemaType() + "'"
      );
   }
   ...
   public void copyCurrentEvent(JsonParser p) throws IOException {
      ...
      switch (t.id()) {
         ...
         case 6:
            ...
            break;
         case 7:
            JsonParser.NumberType nx = p.getNumberType();
            if (nx == JsonParser.NumberType.INT) {
               this.writeNumber(p.getIntValue());
            } else if (nx == JsonParser.NumberType.BIG_INTEGER) {
   ...
   protected final void _verifyOffsets(int arrayLength, int offset, int length) {
      if (offset < 0 || offset + length > arrayLength) {
         throw new IllegalArgumentException(
            String.format("invalid argument(s) (offset=%d, length=%d) for input array of %d element", offset, length, arrayLength)
         );
      }
   }
   ...
   protected void _writeSimpleObject(Object value) throws IOException {
      if (value == null) {
         ...
         throw new IllegalStateException(
            "No ObjectCodec defined for the generator, can only serialize simple wrapper types (type passed " + value.getClass().getName() + ")"
         );
      }
   }
   ...
   public enum Feature {
      ...
      public static int collectDefaults() {
         ...
         for (JsonGenerator.Feature f : values()) {
