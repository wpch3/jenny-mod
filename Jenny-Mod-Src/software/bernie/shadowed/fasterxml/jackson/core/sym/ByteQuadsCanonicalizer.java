...
public final class ByteQuadsCanonicalizer {
   ...
   private void mergeChild(ByteQuadsCanonicalizer.TableInfo childState) {
      ...
      ByteQuadsCanonicalizer.TableInfo currState = this._tableInfo.get();
   ...
   public String findName(int[] q, int qlen) {
      if (qlen < 4) {
         ...
         return hash == hashArea[offset2] && len2 == qlen && this._verifyLongName(q, qlen, hashArea[offset2 + 1])
            ? this._names[offset2 >> 2]
            : this._findSecondary(offset, hash, q, qlen);
      }
   }
