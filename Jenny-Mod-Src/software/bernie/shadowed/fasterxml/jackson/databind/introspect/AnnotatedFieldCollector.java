...
public class AnnotatedFieldCollector extends CollectorBase {
   ...
   public static List<AnnotatedField> collectFields(
      AnnotationIntrospector intr, TypeResolutionContext tc, ClassIntrospector.MixInResolver mixins, TypeFactory types, JavaType type
   ) {
      ...
      for (Class<?> mixin : ClassUtil.findSuperClasses(mixInCls, targetClass, true)) {
         for (Field mixinField : ClassUtil.getDeclaredFields(mixin)) {
            if (this._isIncludableField(mixinField)) {
               ...
               AnnotatedFieldCollector.FieldBuilder b = fields.get(name);
               if (b != null) {
