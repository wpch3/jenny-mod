...
public class TokenBuffer extends JsonGenerator {
      ...
      while (true) {
         ...
         switch (t) {
            ...
            case END_ARRAY:
               ...
               break;
            case FIELD_NAME:
               Object obx = segment.get(ptr);
               if (obx instanceof SerializableString) {
                  gen.writeFieldName((SerializableString)obx);
               } else {
                  gen.writeFieldName((String)obx);
               }
               break;
            case VALUE_STRING:
            ...
            case VALUE_NUMBER_INT:
               Object nx = segment.get(ptr);
               if (nx instanceof Integer) {
                  gen.writeNumber((Integer)nx);
               } else if (nx instanceof BigInteger) {
                  gen.writeNumber((BigInteger)nx);
               } else if (nx instanceof Long) {
                  gen.writeNumber((Long)nx);
               } else if (nx instanceof Short) {
                  gen.writeNumber((Short)nx);
               } else {
                  gen.writeNumber(((Number)nx).intValue());
               }
               break;
               ...
               if (n instanceof Double) {
                  ...
                  if (!(n instanceof String)) {
                     throw new JsonGenerationException(
                        String.format("Unrecognized value type for VALUE_NUMBER_FLOAT: %s, cannot serialize", n.getClass().getName()), gen
                     );
                  }
   ...
   public JsonGenerator enable(JsonGenerator.Feature f) {
      this._generatorFeatures = this._generatorFeatures | f.getMask();
      return this;
   }
   ...
   protected final void _append(JsonToken type) {
      TokenBuffer.Segment next = this._hasNativeId
         ? this._last.append(this._appendAt, type, this._objectId, this._typeId)
         : this._last.append(this._appendAt, type);
      if (next == null) {
         this._appendAt++;
      } else {
         this._last = next;
   ...
   protected final void _append(JsonToken type, Object value) {
      TokenBuffer.Segment next = this._hasNativeId
         ? this._last.append(this._appendAt, type, value, this._objectId, this._typeId)
         : this._last.append(this._appendAt, type, value);
      if (next == null) {
         this._appendAt++;
      } else {
         this._last = next;
   ...
   protected static final class Parser extends ParserMinimalBase {
      ...
      public final Number getNumberValue() throws IOException {
         ...
         if (value instanceof Number) {
            ...
            return str.indexOf(46) >= 0 ? Double.parseDouble(str) : Long.parseLong(str);
         } else if (value == null) {
            return null;
         } else {
