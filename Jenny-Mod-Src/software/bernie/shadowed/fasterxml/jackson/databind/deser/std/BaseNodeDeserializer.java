...
abstract class BaseNodeDeserializer<T extends JsonNode> extends StdDeserializer<T> {
      ...
      for (; key != null; key = p.nextFieldName()) {
         ...
         if (old != null) {
            if (old instanceof ObjectNode) {
               JsonNode newValue = this.updateObject(p, ctxt, (ObjectNode)old);
               if (newValue != old) {
            ...
            if (old instanceof ArrayNode) {
               JsonNode newValue = this.updateArray(p, ctxt, (ArrayNode)old);
               if (newValue != old) {
                  node.set(key, newValue);
               }
               continue;
   ...
   protected final JsonNode _fromInt(JsonParser p, DeserializationContext ctxt, JsonNodeFactory nodeFactory) throws IOException {
      ...
      if (nt == JsonParser.NumberType.INT) {
         ...
         return nt == JsonParser.NumberType.LONG ? nodeFactory.numberNode(p.getLongValue()) : nodeFactory.numberNode(p.getBigIntegerValue());
      }
   }
   ...
   protected final JsonNode _fromFloat(JsonParser p, DeserializationContext ctxt, JsonNodeFactory nodeFactory) throws IOException {
      ...
      if (nt == JsonParser.NumberType.BIG_DECIMAL) {
         ...
         return p.isNaN() ? nodeFactory.numberNode(p.getDoubleValue()) : nodeFactory.numberNode(p.getDecimalValue());
      } else {
   ...
   protected final JsonNode _fromEmbedded(JsonParser p, DeserializationContext ctxt, JsonNodeFactory nodeFactory) throws IOException {
      ...
      if (ob == null) {
         ...
         if (type == byte[].class) {
            ...
            return ob instanceof JsonNode ? (JsonNode)ob : nodeFactory.pojoNode(ob);
         }
      }
   }
}
