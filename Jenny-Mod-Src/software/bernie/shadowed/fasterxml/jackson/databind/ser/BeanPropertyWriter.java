...
public class BeanPropertyWriter extends PropertyWriter implements Serializable {
   ...
   public void assignSerializer(JsonSerializer<Object> ser) {
      if (this._serializer != null && this._serializer != ser) {
         throw new IllegalStateException(
            String.format("Cannot override _serializer: had a %s, trying to set to %s", ClassUtil.classNameOf(this._serializer), ClassUtil.classNameOf(ser))
         );
      }
   ...
   public void assignNullSerializer(JsonSerializer<Object> nullSer) {
      if (this._nullSerializer != null && this._nullSerializer != nullSer) {
         throw new IllegalStateException(
            String.format(
               "Cannot override _nullSerializer: had a %s, trying to set to %s", ClassUtil.classNameOf(this._nullSerializer), ClassUtil.classNameOf(nullSer)
            )
         );
      }
   ...
   public boolean wouldConflictWithName(PropertyName name) {
      return this._wrapperName != null ? this._wrapperName.equals(name) : name.hasSimpleName(this._name.getValue()) && !name.hasNamespace();
   }
