...
public class PotatoArmorItem extends GeoArmorItem implements IAnimatable {
   ...
   private <P extends IAnimatable> PlayState predicate(AnimationEvent<P> event) {
      ...
      if (livingEntity instanceof EntityArmorStand) {
         ...
         client.func_184209_aF().forEach(x -> equipmentList.add(x.func_77973_b()));
         List<Item> armorList = equipmentList.subList(2, 6);
         boolean isWearingAll = armorList.containsAll(
            Arrays.asList(ItemRegistry.POTATO_BOOTS, ItemRegistry.POTATO_LEGGINGS, ItemRegistry.POTATO_CHEST, ItemRegistry.POTATO_HEAD)
         );
