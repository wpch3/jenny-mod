...
public class AnimationProcessor<T extends IAnimatable> {
   ...
   public void tickAnimation(IAnimatable entity, Integer uniqueID, double seekTime, AnimationEvent event, MolangParser parser, boolean crashWhenCantFindBone) {
      ...
      for (Entry<String, DirtyTracker> tracker : modelTracker.entrySet()) {
         ...
         if (saveSnapshot == null) {
            if (crashWhenCantFindBone) {
               throw new RuntimeException(
                  "Could not find save snapshot for bone: "
                     + tracker.getValue().model.getName()
            ...
            if (!tracker.getValue().hasRotationChanged) {
               ...
               double percentageReset = Math.min((seekTime - saveSnapshot.mostRecentResetRotationTick) / resetTickLength, 1.0);
            ...
            if (!tracker.getValue().hasPositionChanged) {
               ...
               double percentageReset = Math.min((seekTime - saveSnapshot.mostRecentResetPositionTick) / resetTickLength, 1.0);
            ...
            if (!tracker.getValue().hasScaleChanged) {
               ...
               double percentageReset = Math.min((seekTime - saveSnapshot.mostRecentResetScaleTick) / resetTickLength, 1.0);
