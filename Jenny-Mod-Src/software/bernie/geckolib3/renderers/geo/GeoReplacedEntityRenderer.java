...
public abstract class GeoReplacedEntityRenderer<T extends IAnimatable> extends Render<EntityLivingBase> implements IGeoRenderer {
   ...
   public void doRender(EntityLivingBase entity, double x, double y, double z, float entityYaw, float partialTicks) {
      ...
      if (shouldSit && entity.func_184187_bx() instanceof EntityLivingBase) {
         ...
         netHeadYaw = f1 - f;
         float f3 = MathHelper.func_76142_g(netHeadYaw);
         if (f3 < -85.0F) {
            f3 = -85.0F;
         }
      ...
      if (this.modelProvider instanceof IAnimatableModel) {
         this.modelProvider.setLivingAnimations(this.animatable, this.getUniqueID((T)entity), predicate);
      }
      ...
      Color renderColor = this.getRenderColor((T)entity, partialTicks);
      ...
      if (entity instanceof EntityPlayer && !((EntityPlayer)entity).func_175149_v()) {
         for (GeoLayerRenderer layerRenderer : this.layerRenderers) {
            layerRenderer.func_177141_a((T)entity, limbSwing, limbSwingAmount, partialTicks, f7, netHeadYaw, headPitch, 0.0625F);
         }
      }
   ...
   protected void applyRotations(EntityLivingBase entityLiving, float ageInTicks, float rotationYaw, float partialTicks) {
      ...
      if (entityLiving.field_70725_aQ > 0) {
         ...
         if (("Dinnerbone".equals(s) || "Grumm".equals(s))
            && (!(entityLiving instanceof EntityPlayer) || ((EntityPlayer)entityLiving).func_175148_a(EnumPlayerModelParts.CAPE))) {
            GlStateManager.func_179137_b(0.0, entityLiving.field_70131_O + 0.1F, 0.0);
   ...
   protected void renderLeash(EntityLiving entityLivingIn, double x, double y, double z, float entityYaw, float partialTicks) {
      ...
      if (entity != null) {
         y -= (1.6 - entityLivingIn.field_70131_O) * 0.5;
         Tessellator tessellator = Tessellator.func_178181_a();
         BufferBuilder bufferbuilder = tessellator.func_178180_c();
         double d0 = this.interpolateValue(entity.field_70126_B, entity.field_70177_z, partialTicks * 0.5F) * (float) (Math.PI / 180.0);
         double d1 = this.interpolateValue(entity.field_70127_C, entity.field_70125_A, partialTicks * 0.5F) * (float) (Math.PI / 180.0);
         ...
         double d7 = this.interpolateValue(entity.field_70167_r + entity.func_70047_e() * 0.7, entity.field_70163_u + entity.func_70047_e() * 0.7, partialTicks)
            - d4 * 0.5
            - 0.25;
         double d8 = this.interpolateValue(entity.field_70166_s, entity.field_70161_v, partialTicks) - d3 * 0.7 + d2 * 0.5 * d5;
         double d9 = this.interpolateValue(entityLivingIn.field_70760_ar, entityLivingIn.field_70761_aq, partialTicks) * (float) (Math.PI / 180.0)
            + (Math.PI / 2);
         d2 = Math.cos(d9) * entityLivingIn.field_70130_N * 0.4;
         d3 = Math.sin(d9) * entityLivingIn.field_70130_N * 0.4;
         ...
         for (int j = 0; j <= 24; j++) {
            ...
            float f3 = j / 24.0F;
            bufferbuilder.func_181662_b(x + d13 * f3 + 0.0, y + d14 * (f3 * f3 + f3) * 0.5 + ((24.0F - j) / 18.0F + 0.125F), z + d15 * f3)
               .func_181666_a(f, f1, f2, 1.0F)
               .func_181675_d();
            bufferbuilder.func_181662_b(x + d13 * f3 + 0.025, y + d14 * (f3 * f3 + f3) * 0.5 + ((24.0F - j) / 18.0F + 0.125F) + 0.025, z + d15 * f3)
               .func_181666_a(f, f1, f2, 1.0F)
               .func_181675_d();
         }
         ...
         for (int k = 0; k <= 24; k++) {
            ...
            float f7 = k / 24.0F;
            bufferbuilder.func_181662_b(x + d13 * f7 + 0.0, y + d14 * (f7 * f7 + f7) * 0.5 + ((24.0F - k) / 18.0F + 0.125F) + 0.025, z + d15 * f7)
               .func_181666_a(f4, f5, f6, 1.0F)
               .func_181675_d();
            bufferbuilder.func_181662_b(x + d13 * f7 + 0.025, y + d14 * (f7 * f7 + f7) * 0.5 + ((24.0F - k) / 18.0F + 0.125F), z + d15 * f7 + 0.025)
               .func_181666_a(f4, f5, f6, 1.0F)
               .func_181675_d();
         }
